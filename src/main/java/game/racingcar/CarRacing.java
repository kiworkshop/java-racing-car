...
public class CarRacing {
    ...
    public static void main(String[] args) {
        ...
        while (!service.isValidCarNames(carNames)) {
            String message = String.format("자동차 이름을 다시 입력해 주세요. (공백 또는 %s 자 초과)", CarRacingService.MAX_CAR_NAME_COUNT);
        ...
        while (!service.isValidGameCount(inputGameCount)) {
            String message = String.format("게임 횟수는 %s 회를 넘을 수 없습니다.", CarRacingService.MAX_GAME_COUNT);
